...
class listar_categorias(ListView):
    ...
    def get_queryset(self):
        menu_pk = self.kwargs.get('pk')
        return Categoria.objects.filter(menu_id=menu_pk)

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['menu'] = get_object_or_404(Menu, pk=self.kwargs.get('pk'))
        return context

class listar_productos(ListView):
    model = Producto
    template_name = 'admin/menu/listar_productos.html'
    context_object_name = 'producto'
    paginate_by = 10

    def get_queryset(self):
        categoria_pk = self.kwargs.get('pk')
        return Producto.objects.filter(categoria_id=categoria_pk)

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['categoria'] = get_object_or_404(Categoria, pk=self.kwargs.get('pk'))
        return context
...
def crear_producto(request):
    ...
    return render(request, 'admin/menu/crear_productos.html', {'form': form})
...
class edicion_producto(UpdateView):
    model = Producto
    form_class = ProductoForm
    template_name = 'admin/menu/editar_productos.html'
    success_url = reverse_lazy('listar_menu')
...
def eliminar_producto(request, pk):
    producto = get_object_or_404(Menu, pk=pk)
    producto.delete()
    return redirect('listar_menu')
